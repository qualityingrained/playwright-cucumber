class InventoryPage {
    ...
    async assertSorting(value) {
        if (['lohi', 'hilo'].includes(value)) {
            var listOfPrices = await page.locator('div.inventory_item_price').all()
            ...
            for (var i = 0; i < (listOfPrices.length - 1); i++) {
                if (value == 'lohi') {
                    ...
                    expect(listOfPrices[i]).toBeGreaterThanOrEqual(listOfPrices[i + 1])
                }
            }
        }
        else {
            ...
            if (value == 'az') {
                for (var i = 0; i < (listOfTitles.length - 1); i++) {
                    ...
                    expect(listOfTitles[i+1].localeCompare(listOfTitles[i])).toBe(1)
                }
            }
            else {
                for (var i = 0; i < (listOfTitles.length - 1); i++) {
                    // compares the A-Z sorting of strings, returns 1 if i comes after i+1
                    expect(listOfTitles[i].localeCompare(listOfTitles[i+1])).toBe(1)
                }
            }
        }
    }
