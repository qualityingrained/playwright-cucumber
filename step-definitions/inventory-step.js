...
When(
	"I sort the products based on price {string}",
	async function (sorting_option) {
...
When(
	"I click on Add to Cart for product {string}",
...
Then(
	"I can see a title, picture, description, price and a buy button",
	async function () {
		await inventoryPage.assertSingleProductUIElements(
...
Then(
	"I can see the products sorted based on the price {string}",
	async function (sorting_option) {
		await inventoryPage.assertInventoryListIsDisplayed();
